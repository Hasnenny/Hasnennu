...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "فالكيري"):
        texting = ["اؤمر " + (get_db_botname() or "فالكيري") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "فالكيري") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "فالكيري") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "سورس بلاك", url=f"https://t.me/aa_ggc" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information()[1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 Black](t.me/aa_ggc)
么 [𝗔𝗦𝗞 𝗧𝗢 𝗠𝗘](t.me/aa_ggc)
么 [Black](t.me/OOQ_OC)
╰──── • ◈ • ────╯
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "بلاك":
        s = await c.get_users("7115839212")
        ss = (await c.get_chat("7115839212")).bio
        sender_name = f"[{m.from_user.first_name}](tg://user?id={m.from_user.id})"
        name_chat = m.chat.title
        id_chat = m.chat.id
        if m.chat.type == enums.ChatType.SUPERGROUP:
        ...
        if m.chat.username:
            ...
            try:
                ...
                link_group = "لايوجد"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(f"{s.first_name}", user_id=7115839212)],
        ])
        try:
            await c.get_chat_photos("7115839212", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
            await c.send_message("7115839212", f"❤️╖ نداء لك ايها المبرمج\n📟╢ بواسطة {sender_name}\n📆╢ يوم *{today}*\n🕑╢ الساعه *{clock}*\n💌╢ اسم الكروب {name_chat}\n🔰╢ ايدي الكروب *{id_chat}*\n⚙️╢ عدد اعضاء الكروب *{num_member}*\n⛓╢ رابط المسج {message_link}\n🔍╜ الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
            async for photo in c.get_chat_photos("7115839212", limit=1):
